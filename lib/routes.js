...
module.exports = function(app, io) {
	...
	// Timeline API routes
	app.get('/api/timeline', _verify, timeline.getEmptyTimeline);
	...
	io.on('connection', function(socket) {
		...
		socket.on('finance-add', function(msg) {
			...
			finances.addFinance(room, msg.data, function(err, result) {
				...
				if(err) {
					...
					timeline.getTimelineItem(result, null, function(err, item) {
						//iolog.debug("add finance, receive item");
		...
		socket.on('finance-modify', function(msg) {
			...
			finances.updateFinance(msg.data, function(err, result) {
				...
				iolog.debug("Finance modified", result);
		...
		socket.on('finance-disable', function(msg) {
			...
			finances.removeFinance(msg.data, function(err, result) {
				if(err) {
					returnModel.error = err;
				} else {
		...
		socket.on('timeline', function() {
			...
			finances.getFinances(room, function(err, finance) {
				...
				timeline.getTimelineItem(finance, params, function(err, item) {
		...
		socket.on('timeline-modify-item', function(msg) {
			...
			timeline.modifyTimelineItem(msg.data, function(err, finance) {
				if(err) {
					returnModel.error = err;
					socket.emit('error', returnModel);
				} else {
					timeline.getTimelineItem(finance, null, function(err, item) {
						if(err) {
							returnModel.error = err;
							socket.emit('error', returnModel);
						} else {
