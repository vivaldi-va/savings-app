...
module.exports = function(app, io) {
	...
	/*	io.sockets
			.on('connection', sioJwt.authorize({
				secret: config.auth.jwt.secret,
			...
			.on('authenticated', function(socket) {
				...
				socket.on('add-finance', function(finance) {
					iolog.debug('add-finance', finance);
					finances.addFinance(room, finance, function(err, result) {
						if(err) {
							socket.emit('add-finance--error', err);
						} else {
