		...
		.then(function(exists) {
			...
			if(exists) {
				...
				User.create({
					...
					token = require('crypto').createHash('md5').update(new Date().getTime() + req.body.email).digest("hex");
					...
					Verify.create({
						...
						transporterOptions= {
							...
							html: "Hello, \n\r" +
								"<p>Thanks for creating an account on PennyJar. You should verify your account, to allow for account recovery.</p>" +
						...
						transporter.sendMail(transporterOptions, ok(next, function(info) {
							log.info("Email verification message sent to %s", req.body.email);
