...
function _findNextFinanceDate(financeDates, financeDate) {


	log.info('DEBUG', financeDates, financeDate);
	var nextDate;
	for(var i = 0; i < financeDates.length; i++) {
		if(financeDates[i] === financeDate) {
			nextDate = financeDates[i+1];
			break;
		}
	}
...
function _getDateObject(date) {
	log.info('UPDATE TIMELINE', "Get date object", date);
	var splitDate = date.split('/');
	return new Date(splitDate[2], splitDate[1], splitDate[0]);
}


function _getFinance(id) {
	var dfd = q.defer();
	db.getConnection(function(err, conn) {
		if(err) dfd.reject(err);

		var sql = "SELECT * FROM finances WHERE id = " + id;
		log.info('UPDATE TIMELINE', "add new finance", sql);
		conn.query(sql, function(err, result) {
			conn.release();
			if(err) dfd.reject(err);
			log.info('QUERY', "Get old finance item info", result);
...
function _getMysqlDate(date) {

	if(typeof date === 'object') {
		return moment(date).format('YYYY-MM-DD');
	} else {
		date = date.split('/');
		return moment(new Date(date[2], date[1]-1, date[0])).format('YYYY-MM-DD');
	}
}
...
exports.updateTimelineItem = function(req, res) {
	var model = {
		"success": false,
		"error": null,
		"message": null,
	...
	log.info('DEBUG', "Body content", req.body);
	...
	req.assert('amount', "Admount missing").notEmpty();
			...
			function(oldFinance) {
				db.getConnection(function(err, conn) {
					if(err) {
						...
						if(req.body.interval === 'once') {
							log.info('UPDATE TIMELINE', "interval once");
							sql = 'UPDATE finances SET amount =  ' + req.body.amount + ', name = "' + req.body.name + '" WHERE id = req.body.id;';
							log.info('UPDATE TIMELINE', "update single finance", sql);
							conn.query(sql, function(err, result) {
								if(err) {
									model.error = err;
									res.send(model);
							...
							var nextFinanceDate = _getDateObject(_findNextFinanceDate(req.body.interval_dates, req.body.date));

							conn.query("UPDATE finances SET `active` = 0, `disabled` = " + _getMysqlDate(new Date()) +  " WHERE id = " + req.body.id,
								function(err, result) {
									log.info('UPDATE TIMELINE', "Disable old finance");
									if(err) {
										model.error = err;
										log.warn('UPDATE TIMELINE FAILED', "Disable old finance");
										res.send(model, 500);
										return;
									}


									sql = "INSERT INTO finances (`id`, `userid`, `created`, `active`, `name`, `type`, `amount`, `duedate`, `interval`, `description`, `disabled`) " +
										"VALUES(null, "+req.signedCookies.saIdent+", null, 1, \"" + req.body.name + "\", " + req.body.type + ", " + req.body.amount + ", \"" + _getMysqlDate(req.body.date) + "\", \"once\", \"" + req.body.description + "\", null);"
									log.info('UPDATE TIMELINE', "add new finance", sql);
									conn.query(sql,
										function(err, result) {
											log.info('UPDATE TIMELINE', "Create one time finance for the date specified");
											if(err) {
												model.error = err;
												log.warn('UPDATE TIMELINE FAILED', "Create new finance");
												res.send(model, 500);
												return;
											}
											...
											sql = "INSERT INTO finances " +
												...
												"VALUES(null, " + req.signedCookies.saIdent + ", null, 1, \"" + oldFinance.name + "\", " + oldFinance.type + ", " + oldFinance.amount + ", \"" + _getMysqlDate(nextFinanceDate) + "\", \"" + oldFinance.interval + "\", \"" + oldFinance.description + "\", null);";
											log.info('UPDATE TIMELINE', "add new finance", sql);
											conn.query(sql,
												function (err, result) {
													conn.release();
													if(err) {
														log.warn('UPDATE TIMELINE FAILED', "resume old finance");
														model.error = err;
														res.send(model, 500);
													} else {
														model.success = true;
