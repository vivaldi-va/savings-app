	...
	Joi.validate(req.body, schema, {abortEarly: false}, function(err, validated) {
		if (err) {
			if(xhr) {
				...
				res.render('login.jade', {error: 'incorrect login details'});
			}
			return;
		}

		// check user exists -> grab their info
		userUtil.emailExists(validated.email, function(err, data) {

			if(!data) {
				if(xhr) {
					log.debug('returning xhr error');
			...
			bcrypt.compare(validated.password, data.passhash, function(err, matches) {
				log.info("password matches?", matches);
				if(!matches) {
					log.debug('DEBUG:', "pass is bad", xhr);

					if(xhr) {
						log.debug('sending xhr response');
						res.status(401).send("ERR_BAD_PASS");
					} else {
						log.debug('re-rendering login.jade');
						res.render('login.jade', {error: 'incorrect login details'});
					}
...
exports.create = function(req, res) {
	...
	Joi.validate(req.body, schema, {abortEarly: false}, function(err, validated) {
		...
		userUtil.emailExists(validated.email, function(err, exists) {
			...
			if(exists) {
				log.debug("user exists", validated.email);
				res.status(401).send("ERR_USER_EXISTS");
				return;
			}
...
exports.deleteTestUser = function(req, res, next) {
	User.findOne({email: "email@email.com"}, function(err, result) {
		if(!!result) {
			User.remove({email: "email@email.com"}, function(err) {
				if(err) {
					res.status(500).end();
					return;
				}
